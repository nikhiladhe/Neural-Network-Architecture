...
class ThreeLayerConvNet(object):
    ...
    def __init__(self, input_dim=(3, 32, 32), num_filters=32, filter_size=7,
        ...
        if self.use_batchnorm:
            ...
            bn_param1 = {'mode': 'train',                # spatial batchnorm
                         'running_mean': np.zeros(F),
                         'running_var': np.zeros(F)}
            gamma1 = np.ones(F)                          # spatial batchnorm
            beta1 = np.zeros(F)                          # spatial batchnorm

            bn_param2 = {'mode': 'train',                # normal batchnorm
                         'running_mean': np.zeros(Hh),
                         'running_var': np.zeros(Hh)}
            gamma2 = np.ones(Hh)                         # normal batchnorm
            beta2 = np.zeros(Hh)                         # normal batchnorm
    ...
    def loss(self, X, y=None):
        ...
        w = W1
        b = b1
        #print 'shape of input to conv layer is=',x.shape
        if self.use_batchnorm:
            beta = beta1
            gamma = gamma1
            bn_param = bn_param1
        ...
        #print 'shape of out of conv layer is=',conv_layer.shape
